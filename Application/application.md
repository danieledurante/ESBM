...
Also in this case, **save the output** in the file `Posterior_Attributes.RData`.
...
# plot Figure 7
ggplot(data_measure, aes(x=X1, y=X2,color=group,fill=group)) + geom_point(aes(size=(size^1.5),shape=shape),stroke = 1)+theme_bw()+ theme(legend.position = "none")+ scale_size_continuous(range = c(0,16))+ scale_color_manual(values=as.character(color_points))+ scale_fill_manual(values=alpha(as.character(color_points),0.3))+xlim(-5,260)+ylim(0.35,1.05)+xlab("Betweenness")+ylab("Local Transivity")+scale_shape_manual(values=c(21, 22,2,3))
```
