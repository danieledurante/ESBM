...
Upload the data
================
To start the analysis, **set the working directory** where the simulated network `network_3.RData`, and the source codes `esbm.R` and `stirling.cpp` are located. Once this has been done, **clean the workspace, and load the data along with useful** `R` **packages**.
...
# save the output
save(Z_DP,l_y_DP,Z_PY,l_y_PY,Z_GN,l_y_GN,Z_DM,l_y_DM,file="Posterior_No_Attributes3.RData")
